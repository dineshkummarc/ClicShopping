...
<div class="contentBody">
  ...
  <table
    ...
    <tr>
      ...
      <th data-field="username"
          data-sortable="true"><?php echo $CLICSHOPPING_MCP->getDef('table_heading_api_username'); ?></th>
      <th data-field="key"
          class="text-center"><?php echo $CLICSHOPPING_MCP->getDef('table_heading_mcp_key_text'); ?></th>
      <th data-field="status" data-sortable="true"
          class="text-center"><?php echo $CLICSHOPPING_MCP->getDef('table_heading_api_status'); ?></th>
      <th data-field="date_added" data-sortable="true"
          class="text-center"><?php echo $CLICSHOPPING_MCP->getDef('table_heading_api_date_added'); ?></th>
      <th data-field="date_modified" data-sortable="true"
          class="text-center"><?php echo $CLICSHOPPING_MCP->getDef('table_heading_api_date_modified'); ?></th>
    ...
    <tbody>
    <?php
    $Qapi = $CLICSHOPPING_MCP->db->prepare('select SQL_CALC_FOUND_ROWS mcp_id,
                                                                        username,
                                                                        mcp_key,
                                                                        status,
                                                                        date_added,
    ...
    if ($listingTotalRow > 0) {
      while ($Qapi->fetch()) {
        ?>
        <td><?php echo $Qapi->valueInt('mcp_id'); ?></td>
        <td><?php echo $Qapi->valueProtected('username'); ?></td>
        <td><?php echo substr($Qapi->valueProtected('mcp_key'), -40, 40) . '...'; ?></td>
        <td class="text-center">
          <?php
          if ($Qapi->valueInt('status') == 1) {
            echo HTML::link($CLICSHOPPING_MCP->link('MCP&SetFlag&flag=0&cID=' . $Qapi->valueInt('mcp_id') . '&page=' . $page), '<i class="bi-check text-success"></i>');
          } else {
            echo HTML::link($CLICSHOPPING_MCP->link('MCP&SetFlag&flag=1&cID=' . $Qapi->valueInt('mcp_id') . '&page=' . $page), '<i class="bi bi-x text-danger"></i>');
          }
          ?>
        </td>
        <td><?php echo DateTime::toShort($Qapi->valueProtected('date_added')); ?></td>
        <td><?php echo DateTime::toShort($Qapi->valueProtected('date_modified')); ?></td>
        <td class="text-end">
          <div class="btn-group d-flex justify-content-end" role="group" aria-label="buttonGroup">
            <?php
            echo HTML::link($CLICSHOPPING_MCP->link('Edit&page=' . $page . '&cID=' . $Qapi->valueInt('mcp_id')), '<h4><i class="bi bi-pencil" title="' . $CLICSHOPPING_MCP->getDef('icon_edit') . '"></i></h4>');
            echo '&nbsp;';
            echo HTML::link($CLICSHOPPING_MCP->link('Api&Delete&page=' . $page . '&cID=' . $Qapi->valueInt('mcp_id')), '<h4><i class="bi bi-trash2" title="' . $CLICSHOPPING_MCP->getDef('icon_delete') . '"></i></h4>');
            echo '&nbsp;';
            ?>
          </div>
        </td>
  ...
  if ($listingTotalRow > 0) {
    ...
    <div class="row">
      <div class="col-md-12">
        <div
          class="col-md-6 float-start pagenumber hidden-xs TextDisplayNumberOfLink"><?php echo $Qapi->getPageSetLabel($CLICSHOPPING_MCP->getDef('text_display_number_of_link')); ?></div>
        <div
