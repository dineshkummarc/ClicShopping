 ...
 */
class AIProcess implements \ClicShopping\OM\Modules\HooksInterface
{
  public mixed $app;
  ...
  private function fetchOrderData(int $orderId): array
  {
    ...
    // Fetch order total
    $totalQuery = $ClicShopping_Db->prepare('
      SELECT value 
    ...
    $productsQuery = $ClicShopping_Db->prepare('
      SELECT op.*, 
             p.products_model,
             pd.products_name
  ...
  private function storeInsights(int $orderId, array $insights, string $insightType): int
  {
    ...
    if ($existing) {
      ...
      return (int)$ClicShopping_Db->lastInsertId();
    }
  }
